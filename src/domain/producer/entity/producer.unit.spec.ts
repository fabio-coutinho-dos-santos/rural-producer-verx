...
describe("Producer unit tests", () => {
  ...
  it("should throw an error when document is invalid", () => {
    expect(() => {
      new Producer("Producer name", "123456781");
    }).toThrow("Invalid document format");

    expect(() => {
      new Producer("Producer name", "39.443.878/0001-31");
    }).toThrow("Invalid CNPJ");

    expect(() => {
      new Producer("Producer name", "213.101.950-52");
    }).toThrow("Invalid CPF");

    expect(() => {
      new Producer("Producer name", "1234567891A");
    }).toThrow("Invalid document format");

    expect(() => {
      new Producer("Producer name", "");
    }).toThrow("CNPJ or CPF is required");
  });

  it("should throw an error when name is invalid", () => {
    expect(() => {
      new Producer("", "12345678911");
