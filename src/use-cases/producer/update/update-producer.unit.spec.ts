...
import { inputUpdateProducerStub, producerStub, validUuidFormat } from "../../@shared/tests/stub";
...
describe('Update producer use case', () => {
  ...
  describe('buildNewProducer function', () => {
    it('should return a producer with valid data', async () => {
      const producerRepository: ProducerRepositoryInterface = ProducerMockRepository(true);
      const updateProducer = new UpdateProducer(producerRepository);
      const producerBuilded = await updateProducer.buildNewProducer(inputUpdateProducerStub(), validUuidFormat())
    ...
    it('should return a not found error with invalid producer id', async () => {
      expect(async () => {
        const producerRepository: ProducerRepositoryInterface = ProducerMockRepository(false);
        const updateProducer = new UpdateProducer(producerRepository);
        const producerBuilded = await updateProducer.buildNewProducer(inputUpdateProducerStub(), validUuidFormat())
  ...
  describe('execute function', () => {
    it('should return a producer with valid data', async () => {
      const producerRepository: ProducerRepositoryInterface = ProducerMockRepository(true);
      const updateProducer = new UpdateProducer(producerRepository);
      const spyBuildNewProducer = await jest.spyOn(updateProducer, 'buildNewProducer')
      const producerUpdated = await updateProducer.execute(inputUpdateProducerStub(), validUuidFormat())
      ...
      expect(spyBuildNewProducer).toHaveBeenCalledWith(inputUpdateProducerStub(), validUuidFormat())
    })
